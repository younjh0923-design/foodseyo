...
C2.1 does not persist raw images; create restaurant, dish-concept, observation, or logical-menu catalogs; implement dish-level reuse; analyze links; change the canonical schema, provider prompt/schema/model defaults, or source/dish fingerprint semantics; or change UI/session behavior. C2.1-E adds only the already frozen ownership and 409/503 cache policy locally, and C2.1-F validates it without expanding scope. C2.1-G completed the separate rollout review and withheld Production approval. The preserved automatic Preview is build provenance only: there is still no Preview/Production database migration or authorization to roll out cache behavior. C2.2-A/A1 change only the future logical model. C2.2-B mirrors this implemented four-table boundary in [database-physical-integrity-contract.md](./database-physical-integrity-contract.md) and adds a non-executable contract for four future structured-menu tables; none of these checkpoints alters C2.1.
