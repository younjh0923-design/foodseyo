...
Canonical `1.1.1` also separates status from provenance and identity scope. A user-entered name without matching source evidence is `likely/user_declared/restaurant`; a source-stated name is `confirmed/source_stated/restaurant`; compatible user and source names are `confirmed/source_and_user`; a mismatch stays `unconfirmed/source_and_user/unknown` with neither identity selected. Branch scope requires preserved branch-specific evidence such as a source-stated address or phone, not a restaurant name or location hint alone.
...
C2.1-E now composes the C2.1-D exact lookup with pre-provider ownership. A valid hit still bypasses provider construction. A confirmed miss acquires one 120-second processing lease before provider construction, while a duplicate polls for a valid snapshot for at most 2 seconds at a 200-millisecond interval. A completed snapshot is reused; an unresolved active owner returns retryable HTTP 409 `ANALYSIS_IN_PROGRESS` with `Retry-After: 2`, and indeterminate ownership returns retryable HTTP 503 `ANALYSIS_TEMPORARILY_UNAVAILABLE`.

Cache/database failures remain uncached fail-open only before an ownership transaction can have created state. From the acquisition attempt onward, ownership is fail-closed: a proposed run UUID is re-read once after an ambiguous outcome, only a proven owner may invoke the provider or persist, and an expired owner is failed as `LEASE_EXPIRED` before a new append-only attempt is created in the same short transaction. Provider execution holds no database transaction. Snapshot insertion and the owned run's `ready` transition remain atomic. C2.1-F independently passed repeated real-PostgreSQL concurrency, rollback, ambiguous-outcome, ownership, snapshot-integrity, and quarantine-failure validation on disposable Development child branches. The local implementation is still not authorized for Preview, Production, or deployment before C2.1-G.
