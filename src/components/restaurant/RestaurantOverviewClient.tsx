  ...
  const filteredDishes = useMemo(() => {
    ...
    return demoRestaurant.dishes.filter((dish) => {
      ...
      const matchesSearch =
        ...
        dish.shortDescription.toLowerCase().includes(normalized) ||
        dish.tasteTags.some((tag) => tag.toLowerCase().includes(normalized));
  ...
  return (
    <MobileShell>
      <article className="min-h-dvh pb-28">
        <PageHeader
          ...
          action={
            <IconButton
              ...
              className={
                favorite
                  ? "border-[var(--accent)] bg-[var(--soft-orange)] text-[var(--accent)]"
                  : undefined
              }
            >
        ...
        <header>
          <div className="relative h-[218px] overflow-hidden bg-[var(--canvas)]">
            <SafeImage
              src={demoRestaurant.imageUrl}
              alt="Northern Thai dishes used as a demo reference"
              imagePosition="50% 46%"
              className="h-full w-full object-cover"
            />
            <span className="absolute bottom-3 left-4 rounded-full bg-[var(--surface)]/95 px-3 py-1.5 text-[10px] font-bold text-[var(--text-secondary)]">
              {demoRestaurant.imageSource}
            </span>
          </div>
          <div className="page-padding pt-6">
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[var(--accent)]">
              Demo restaurant analysis
            </p>
            <h1 className="mt-2 text-[30px] font-bold leading-[1.08] tracking-[-0.04em]">
              {demoRestaurant.name}
            </h1>
            <p className="mt-3 text-sm font-semibold text-[var(--primary)]">
              {demoRestaurant.location} · {demoRestaurant.cuisine} · {demoRestaurant.priceLevel}
            </p>
            <p className="mt-3 text-[15px] leading-6 text-[var(--text-secondary)]">
              {demoRestaurant.shortSummary}
            </p>
          </div>
        </header>
        ...
        <section aria-labelledby="representative-title" className="mt-9">
          <div className="page-padding flex items-end justify-between gap-4">
            <div>
              ...
              <h2 id="representative-title" className="mt-1 text-[23px] font-bold tracking-[-0.03em]">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-5 text-[var(--text-secondary)]">
                  Choose party size, budget and what matters today.
                </p>
              </div>
            </div>
            <PrimaryButton className="mt-4 w-full" onClick={() => setPlannerOpen(true)}>
              Plan this meal
            </PrimaryButton>
          </div>
        </section>
        ...
        <section aria-labelledby="all-menu-title" className="page-padding mt-10">
          ...
          {filteredDishes.length ? (
            ...
            <div className="mt-4 rounded-[20px] border border-dashed border-[var(--border-strong)] p-6 text-center">
              <p className="text-sm font-bold">No menu matches</p>
              <p className="mt-1 text-sm text-[var(--text-secondary)]">Try another category or search term.</p>
            </div>
          )}
        </section>
      </article>
      ...
      <button
        ...
        className="floating-action fixed z-30 flex size-14 items-center justify-center rounded-full border-4 border-[var(--surface)] bg-[var(--primary)] text-white shadow-[0_10px_24px_rgba(33,94,64,0.22)]"
      >
        <MessageCircle aria-hidden="true" size={22} />
        <Sparkles aria-hidden="true" className="absolute right-2 top-2 text-[#f8d5bd]" size={9} />
      </button>
